...
### Fixed
- `autoplay: false` no longer starts/resets the internal periodic autoplay timer when calling `SwiperController.next()` / `move()` (via `needToResetTimer`).
- `SwiperController.startAutoplay()` can still be used to manually force-start autoplay.
